...
export default function ErrorsPage({ projectId, project }: ProjectPageProps) {
  ...
  return <>
    <PageHeader eyebrow="ERROR INTELLIGENCE" title="Evidence before assumptions" description={`${project?.name ?? projectId}의 오류 분석, 검증 이력과 유사 사례를 관리합니다.`} action={<Segmented value={tab} onChange={value=>setTab(value as Tab)} options={[{value:'history',label:'History'},{value:'analyze',label:'Analyze'},{value:'similar',label:'Similar'}]} />} />
