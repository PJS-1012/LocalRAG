...
import { CodeBlock, EmptyState, ErrorNotice, InfoNotice, LoadingState, PageHeader, Panel, StatusBadge } from '../components/ui'
...
export default function RagPage({ projectId, project, overview }: ProjectPageProps) {
  ...
  return <>
    <PageHeader eyebrow="고급 기능 · RAG" title="지식 검색 상세" description={`${project?.name ?? projectId}의 인덱싱된 자료만 사용합니다.`} />
    {overview?.index?.storedChunkCount === 0 && <InfoNotice>아직 검색할 Project 자료가 없습니다. Dashboard에서 현재 Project의 ‘Project 인덱싱’을 실행한 후 질문해주세요.</InfoNotice>}
    <div className="split-workspace">
      ...
      </div><form className="prompt-box" onSubmit={submit}><textarea aria-label="RAG 질문" value={query} onChange={event => setQuery(event.target.value)} onKeyDown={event => submitOnEnter(event, loading || !query.trim())} rows={3} placeholder="Project 자료에 대해 질문하세요" /><div><span>Enter 전송 · Shift+Enter 줄바꿈 · Top-K 5 · threshold 0.45</span><button className="button button-primary" disabled={loading || !query.trim()}>전송</button></div></form></Panel>
