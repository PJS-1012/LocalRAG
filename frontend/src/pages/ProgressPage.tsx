...
export default function ProgressPage({projectId,project}:ProjectPageProps){
  ...
  return <><PageHeader eyebrow="WORKFLOW INTELLIGENCE" title="Project progress" description="완료율을 추정하지 않고 확인 가능한 근거별로 현재 상태를 정리합니다." action={<button className="button button-primary" onClick={run} disabled={loading}>{loading?'분석 중…':'새 분석'}</button>}/>{error&&<ErrorNotice message={error}/>} {loading&&<LoadingState label="Git·Project 자료 수집 및 Local Model 분석 중"/>}
