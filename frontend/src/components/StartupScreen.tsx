...
export default function StartupScreen({ status, retry }: { status: DesktopBackendStatus; retry: () => void }) {
  ...
  return <main className="startup-screen"><section className="panel startup-card" aria-live="polite">
    <span className="eyebrow">LOCALRAG</span><h1>{status.state === 'STARTING' ? 'LocalRAG 준비 중' : '실행 환경 준비 실패'}</h1>
    ...
    <p>{status.detail}</p>
    {status.state !== 'STARTING' && <button className="button button-primary" onClick={retry}>다시 준비</button>}
  </section></main>
}
